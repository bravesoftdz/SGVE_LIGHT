...
uses
  ...
  Dialogs, uPadrao, StdCtrls, Mask, System.StrUtils,
  Grids, DBGrids, DBGridCBN, ComCtrls, Buttons, ExtCtrls, DB, DBClient, Especificacao, RxToolEdit, RxCurrEdit;
...
begin
   ...
   if self.cds.Fields.Count <= 0 then Exit;
...
begin
   ...
   habilitaDesabilitaSheetsSecundarios(pgGeral, true, true);   
...
begin
   ...
   habilitaDesabilitaSheetsSecundarios(pgGeral, false, false);
end;
...
begin
   ...
   else if  (pg.Pages[pg.ActivePageIndex] <> tsConsulta) and
        not (pg.Pages[pg.ActivePageIndex].Enabled) then self.EstadoTela := tetVisualizar;
end;
...
procedure TfrmCadastroPadrao.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (key = VK_ESCAPE) and (self.EstadoTela in [tetIncluir, tetAlterar]) then
    if not confirma('Deseja cancelar '+IfThen(self.EstadoTela = tetIncluir, 'inclusão', 'alteração')+' em andamento?') then
      key := 0;
...
procedure TfrmCadastroPadrao.habilitaDesabilitaSheetsSecundarios(pag: TPageControl; HD, VI: Boolean);
var i:integer;
begin
  for i := 1 to pag.PageCount - 1 do
  begin
    pag.Pages[i].Enabled := HD;
    pag.Pages[i].Visible := VI;
  end;
end;
