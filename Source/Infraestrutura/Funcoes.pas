...
function DataParaParametro(data :TDate) :String;
...
procedure verificaGenerator(generator, tabela :String);
var valorGenerator, valorNaTabela :integer;
begin
  valorGenerator := dm.GetValorGenerator(generator);

  dm.qryGenerica.Close;
  dm.qryGenerica.SQL.Text := 'SELECT max(CODIGO) codigo FROM '+tabela;
  dm.qryGenerica.Open;

  valorNaTabela  := dm.qryGenerica.FieldByName('CODIGO').AsInteger;

  while valorGenerator > valorNaTabela do
  begin
    dm.GetValorGenerator(generator,-1);
    dec(valorGenerator);
  end;
end;
...
function DataParaParametro(data :TDate) :String;
begin
  result := StringReplace(DateToStr(data), '/', '.', [rfReplaceAll]);
